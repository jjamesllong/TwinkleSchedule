...
class EditCourseViewModel : BaseViewModel() {
    ...
    private val _liveTextColor = MutableLiveData<Int>()
    val liveTextColor: LiveData<Int> get() = _liveTextColor
    ...
    override suspend fun onPlace() {
        ...
        viewModelScope.launch(Dispatchers.Default) {
            ...
            launch {
                Pipette.forInt.subscribe(KEY_COURSE_TEXT_COLOR) {
                    withContext(Dispatchers.Main) {
                        _liveTextColor.value = it
                    }
                }
            }
        }
    ...
    fun setScheduleLimit(masterId: Long, endDay: Int, endSection: Int, endWeek: Int) {
    ...
    fun setCourseInfo(
        ...
        _liveColor.value = color
        _liveTextColor.value = textColor
    }

    suspend fun writeCourse(isUpdate: Boolean): Boolean {
        return withContext(Dispatchers.IO) {
            requestCourse()?.let { course ->
                requestCourseDecoration(course.courseId)?.let { decoration ->
                    if (isUpdate) {
                        CourseRepository.updateCourse(course)
                        CourseDecorationRepository.updateCourseDecoration(decoration)
                        true
                    } else {
    ...
    private fun requestCourse(): Course? {
        ...
        val parentScheduleId = if (masterId == 0L) return null else masterId
        val title = _liveTitle.value ?: return null
        val day = _liveDay.value?.toNumber() ?: return null
    ...
    private fun requestCourseDecoration(id: Long): CourseDecoration? {
        ...
        val textColor = _liveTextColor.value ?: return null
        return CourseDecoration(id, color.toStringHex(), textColor.toStringHex())
    }
}